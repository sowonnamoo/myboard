...
function renderTable() {
    ...
    // [이전] 버튼
    if (currentPage > 1) {
        pager.innerHTML += `<span class="cursor-pointer px-2" onclick="goToPage(${currentPage - 1})">[이전]</span>`;
    }

    // 6개 페이지만 보여주기 위한 범위 계산
    let startPage = Math.max(1, currentPage - 2); // 현재 페이지 기준 앞으로 2개
    let endPage = Math.min(totalPages, startPage + 5); // 총 6개가 되도록

    // 만약 끝 페이지가 작으면 시작 페이지를 앞으로 밀어줌
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pager.innerHTML += `<span class="cursor-pointer px-1 ${activeClass}" onclick="goToPage(${i})">${i}</span>`;
    }

    // [다음] 버튼
    if (currentPage < totalPages) {
        pager.innerHTML += `<span class="cursor-pointer px-2" onclick="goToPage(${currentPage + 1})">[다음]</span>`;
    }
}
