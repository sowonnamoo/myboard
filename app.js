...
지금 관리자님의 상황은 "가격 데이터(price)와 상태 데이터(status)가 데이터베이스상에서 꼬여 있고, 이를 출력하는 코드도 수정이 필요한 상태"입니다.

이를 위해 app.js에서 금액 표시 로직과 상태 기반 이미지 제어 로직이 모두 포함된 전체 코드를 정리해 드립니다.

app.js 전체 코드
이 코드를 기존 app.js 파일에 전체 덮어쓰기 하세요.

JavaScript
// 1. 상태에 따라 PNG 오버레이를 제어하는 함수
...
// 2. 상세 페이지 데이터 표시 함수 (이 부분을 꼭 확인하세요!)
// 서버에서 데이터를 가져온 후 이 함수를 실행하세요.
window.renderDetail = function(data) {
    // [중요] 금액은 price 필드에서, 상태는 status 필드에서 각각 가져옵니다.
    // 기존에 글자가 섞여있던 가격 필드를 원래의 숫자로 복구하는 것이 핵심입니다.
    document.getElementById('detail-price').innerText = (data.price || 0) + '원';
    
    // 상태 표시용 요소가 있다면 여기에 추가 (없다면 생략 가능)
    const statusEl = document.getElementById('detail-status');
    if (statusEl) statusEl.innerText = data.status || '대기중';
