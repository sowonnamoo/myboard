...
// [최적화] 삭제된(isDeleted:true) 글이 섞여 있을 때, 예전에는 매번 POSTS_PER_PAGE(6개)씩만
// 가져와서 부족하면 또 6개, 또 6개... 식으로 Firestore 왕복(round-trip)이 여러 번
...
async function fetchValidOrders(targetCount) {
    ...
    while (collected.length < targetCount && !exhausted) {
        ...
        const q = lastVisible
            ? query(collection(db, "boards"), orderBy("createdAt", "desc"), startAfter(lastVisible), limit(fetchLimit))
            : query(collection(db, "boards"), orderBy("createdAt", "desc"), limit(fetchLimit));
        const snapshot = await getDocs(q);
...
setInterval(() => {
    ...
    if (approveBtn && approveBtn.innerText === "조판완료") {
        if (!document.getElementById('reorder-btn')) {
            ...
            reorderBtn.className = 'bg-green-500 text-white px-9 py-3 rounded font-bold hover:bg-green-600 text-2xl';
